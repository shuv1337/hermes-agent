...
class AIAgent:
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        ...
        3. CamelCase -> snake_case (TodoTool -> todo_tool), with optional
           trailing ``_tool`` / ``-tool`` / ``tool`` suffix stripped (applied
           twice so double-tacked suffixes like ``TodoTool_tool`` reduce all
           the way).
        4. Prepend ``mcp_`` (common failure: Claude-style models drop the
           MCP prefix from ``mcp_<server>_<TOOL>`` calls).
        5. Case-insensitive direct match (MCP tool names often contain
           UPPERCASE segments like COMPOSIO_MULTI_EXECUTE_TOOL).
        6. Fuzzy match (difflib, cutoff=0.7, case-insensitive).
        ...
        # 3. CamelCase + tool-suffix recovery — TodoTool / TodoTool_tool /
        #    BrowserClick_tool style class-like emissions from Claude-style
        #    models. Build a candidate set, then strip trailing _tool/-tool/tool
        #    up to twice (handles double-tacked suffixes).
        ...
        for c in cands:
            if not c:
                continue
            if c in self.valid_tool_names:
                return c
            if c.lower() in lower_map:
                return lower_map[c.lower()]

        # 4. MCP prefix recovery — Claude Opus repeatedly drops `mcp_` from
