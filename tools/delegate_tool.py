...
def _resolve_delegation_credentials(
    ...
    if configured_base_url and not requested_provider_switches_backend and not _is_native_sdk_provider:
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "- Subagent model/provider can be selected per call with the 'model' and 'provider' parameters; otherwise children inherit the parent model (plus its fallback chain) unless delegation.provider / delegation.model pins all subagents in config.yaml.\n"
