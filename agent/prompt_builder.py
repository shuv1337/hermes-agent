...
COMPUTER_USE_GUIDANCE = (
    ...
    "time.\n\n"
    "## Preferred workflow\n"
    "1. Pin a target window with `action='capture'` and `app='AppName'` "
    "(e.g. `app='Finder'`). `mode='som'` (default) returns a screenshot "
    "with numbered overlays on every interactable element plus an AX-tree "
    "index that lists role, label, hint, and the AX actions each element "
    "supports. Element actions are cached PER (pid, window_id) — "
    "recapture whenever you switch target windows.\n"
    "2. Click by element index: `action='click', element=14`. The backend "
    "picks the right AX action (`AXPress`, `AXOpen`, etc.) based on what "
    "the captured element advertised. If the element only exposes `AXOpen`, "
    "use `double_click`. For select dropdowns and sliders, use `set_value` "
    "— it writes the value without opening a native menu.\n"
    "3. If `click` returns an `AX*` error or the element exposes no "
    "clickable AX action, fall back to coordinate click — coordinates are "
    "in **window-local screenshot pixels** (top-left origin of the capture "
    "PNG), not global screen coordinates.\n"
    "4. For text input, `action='type', text='...'`. For key combos "
    "`action='key', keys='cmd+s'`. For scrolling `action='scroll', "
    "direction='down', amount=3`. These are the most reliable actions.\n"
    "5. After any state-changing action, re-capture to verify. You can "
    "pass `capture_after=true` to get the follow-up screenshot in one "
    "round-trip.\n\n"
    "## Known backend limits\n"
    "- `middle_click` is not supported — the call returns an explicit "
    "unsupported error.\n"
    "- Element-based `drag` requires a fresh capture whose elements include "
    "bounds. If bounds are missing, use `from_coordinate` / `to_coordinate` "
    "(window-local screenshot pixels).\n"
    "- `focus_app(raise_window=true)` deliberately brings the target window "
    "foreground when cua-driver supports `raise_window`; use it only when "
    "the user explicitly asks for a foreground switch.\n\n"
    "## Background mode rules\n"
    "- Default to `raise_window=false` on `focus_app`. Input routing works "
    "without raising, and foreground switches are disruptive.\n"
    "- When capturing, prefer `app='Safari'` (or whichever app the task "
    "is about) instead of the whole screen — it's less noisy and won't "
    "leak other windows the user has open. Capture without `app=` only "
    "when you want the frontmost on-screen window.\n"
    "- If `capture(app=X)` returns 0 elements / 0x0 window, the app has "
    "no on-screen window matching X. Don't blindly retry against the "
    "frontmost — the backend deliberately refuses to fall back.\n"
