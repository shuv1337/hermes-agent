...
def _manage_thinking_signatures(
    ...
    for idx, m in enumerate(result):
        ...
        # Strip cache_control from thinking blocks except on an intact latest
        # direct-Anthropic assistant turn — Opus 4.8+ requires that turn
        # byte-exact, including cache markers.
        strip_thinking_cache = (
            _is_kimi_family_endpoint(base_url, model)
            or _is_deepseek_anthropic_endpoint(base_url)
            or _is_third_party
            or idx != last_assistant_idx
            or bool(m.get("_thinking_signature_invalidated"))
        )
        if strip_thinking_cache:
            for b in m["content"]:
