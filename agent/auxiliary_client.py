...
# expose 1.05M for the same slugs, while the live Codex catalog reports 272K
# for gpt-5.4/5.5 and 372K for gpt-5.6
# (verified live for 5.4/5.5: a ~330K-token request to
# chatgpt.com/backend-api/codex/responses is rejected with
# ``context_length_exceeded`` while ~250K succeeds; see
# _CODEX_OAUTH_CONTEXT_FALLBACK in model_metadata.py for the live 5.6 value).
# At either ceiling the default 50% compaction trigger fires halfway through —
...
def _is_codex_gpt54_or_gpt55(model: Optional[str], provider: Optional[str] = None) -> bool:
    ...
    via prefix so the override tracks every Codex-capped family (5.4, 5.5,
    ...
    *,
    ...
    Per-model/route overrides:
      ...
      - gpt-5.4 / gpt-5.5 / gpt-5.6 on the Codex OAuth route → 0.85, because
        Codex exposes 272K for 5.4/5.5 and 372K for 5.6, while the default 50%
        trigger still wastes half the window. Gated by ``allow_codex_gpt55_autoraise``
