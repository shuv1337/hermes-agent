...
class TestSignalSyncMessageHandling:
    ...
    async def test_group_sync_sent_promoted_to_inbound(self, monkeypatch):
        ...
        # This fork defaults require_mention=True for group gating (upstream
        # defaults it off). Disable it here so this test exercises the
        # sync-sent -> inbound promotion itself rather than the group
        # mention gate that would otherwise (correctly) drop an unmentioned
        # group message under the fork's stricter default.
        adapter = _make_signal_adapter(
            monkeypatch,
            account="+155****4567",
