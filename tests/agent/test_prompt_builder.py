...
class TestBuildContextFilesPrompt:
    ...
    def test_agents_md_no_git_root_stays_cwd_only(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(prompt_builder, "_find_git_root", lambda _path: None)
        assert prompt_builder._load_agents_md(sub) == ""
...
class TestPromptBuilderConstants:
    ...
    def test_cli_hint_does_not_suggest_media_tags(self):
        ...
        # check that this test is calibrated correctly). Anthropic OAuth
        # blocks the `MEDIA:/absolute/path` literal, so the shared spine
        # names the marker without that exact path form.
        telegram_hint = PLATFORM_HINTS["telegram"]
        assert "MEDIA:" in telegram_hint
        assert "MEDIA:/absolute/path" not in telegram_hint
