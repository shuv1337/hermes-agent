...
# ---------------------------------------------------------------------------
# OAuth tool-name encoding — April 2026 blocklist guard
#
# Anthropic's Claude Code OAuth edge rejects requests whose ``tools[*].name``
# matches ``^mcp_[a-z0-9_]+$`` with a misleading ``out of extra usage`` 400.
# The encoder in agent/anthropic_adapter.py introduces at least one uppercase
# character in the suffix so every name we send falls outside the pattern.
# ---------------------------------------------------------------------------


class TestOAuthToolNameEncoding:
    def test_blocklist_regex_characterization(self):
        """Sanity-check the regex matches the names live probing flagged."""
        from agent.anthropic_adapter import _OAUTH_BLOCKED_TOOL_NAME_RE as R

        # Blocked per live bisect in tests/integration/test_oauth_blocklist_bisect.py
        for blocked in (
            "mcp_foo",
            "mcp_foo_bar",
            "mcp_f",
        ...
        for allowed in (
            ...
            "foo_mcp_bar",          # prefix not at start
        ):
            assert not R.match(allowed), f"regex should NOT block {allowed!r}"

    def test_encode_adds_capitalization_for_unprefixed_hermes_tools(self):
        from agent.anthropic_adapter import _encode_oauth_tool_name as enc

        assert enc("terminal") == "mcp_Terminal"
        assert enc("read_file") == "mcp_Read_file"
        assert enc("browser_back") == "mcp_Browser_back"
        assert enc("ha_call_service") == "mcp_Ha_call_service"

    def test_encode_capitalizes_in_place_for_prefixed_mcp_tools_that_would_trip_filter(self):
        from agent.anthropic_adapter import _encode_oauth_tool_name as enc
    ...
    def test_encode_handles_empty_name(self):
        ...
        from agent.anthropic_adapter import _encode_oauth_tool_name as enc
        assert enc("") == _MCP_TOOL_PREFIX

    def test_every_encoded_name_passes_blocklist_regex(self):
        """Exhaustive guard: for every canonical tool name in the live
        registry, the encoded form must NOT match the blocklist regex."""
        from agent.anthropic_adapter import (
            _OAUTH_BLOCKED_TOOL_NAME_RE,
            _encode_oauth_tool_name,
        )
        from model_tools import get_tool_definitions

        for tool in get_tool_definitions(quiet_mode=True):
            name = tool["function"]["name"]
            encoded = _encode_oauth_tool_name(name)
            assert not _OAUTH_BLOCKED_TOOL_NAME_RE.match(encoded), (
                f"Encoded tool name {encoded!r} (from canonical {name!r}) "
                f"matches the Anthropic OAuth blocklist regex. The encoder is "
                f"not protecting us from the April 2026 filter."
            )

    def test_decode_without_canonical_drops_prefix(self):
        """Legacy callers (no canonical_names argument) keep the old behavior."""
        from agent.anthropic_adapter import _decode_oauth_tool_name as dec
        assert dec("mcp_Terminal") == "terminal"
        assert dec("mcp_Read_file") == "read_file"
        assert dec("no_prefix_here") == "no_prefix_here"

    def test_decode_with_canonical_restores_exact_canonical_name(self):
        """When the caller passes ``canonical_names`` the decoder round-trips
        MCP-style and Hermes-style names to their exact registry form."""
        from agent.anthropic_adapter import (
            _decode_oauth_tool_name as dec,
            _encode_oauth_tool_name as enc,
        )
    ...
    def test_decode_unknown_name_with_canonical_returns_encoded(self):
        """If the decoded candidate isn't in the registry, surface the raw
        encoded form so downstream dispatch raises a clear not-found error
        rather than silently routing to the wrong tool."""
        from agent.anthropic_adapter import _decode_oauth_tool_name as dec
        assert dec("mcp_Fake_tool", canonical_names={"terminal"}) == "mcp_Fake_tool"


class TestBuildAnthropicKwargsOAuthToolEncoding:
    def test_tool_schemas_are_encoded_for_oauth(self):
        """Every tool in ``tools=`` must ship with a non-blocklist name when
        ``is_oauth=True``."""
        ...
        sent_names = [t["name"] for t in kwargs["tools"]]
        assert sent_names == [
            "mcp_Terminal",
            "mcp_Read_file",
        ...
        for name in sent_names:
            assert not _OAUTH_BLOCKED_TOOL_NAME_RE.match(name), (
                f"Encoded tool name {name!r} still matches OAuth blocklist."
            )

    def test_tool_use_blocks_in_history_are_encoded(self):
        """Tool_use blocks in assistant history (replay of past turns) also
        have to be encoded — otherwise the OAuth edge rejects the whole
        request."""
        messages = [
        ...
        assistant_turns = [m for m in kwargs["messages"] if m["role"] == "assistant"]
        assert assistant_turns, "expected an assistant turn with a tool_use block"
        tool_use_names = []
        for m in assistant_turns:
            for block in m.get("content") or []:
                if isinstance(block, dict) and block.get("type") == "tool_use":
                    tool_use_names.append(block["name"])
        assert tool_use_names == ["mcp_Read_file"], (
            f"tool_use block name was not encoded: {tool_use_names}"
        )
        assert kwargs["tools"][0]["name"] == "mcp_Read_file"

    def test_non_oauth_path_does_not_encode_tool_names(self):
        """Regular API-key requests must NOT get the Claude Code tool-name
        encoding. Only the OAuth edge has the filter."""
        tools = [
            {
                "type": "function",
                "function": {
