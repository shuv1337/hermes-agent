          ...
          <div className="mb-3 flex items-center justify-between gap-3">
            <div>
              ...
              <div className="mt-1 text-xs text-muted-foreground">
                Realtime is low-latency speech-to-speech (needs an OpenAI key; falls back to Classic automatically).
                Classic is the local transcribe → think → speak loop.
              </div>
            </div>
            <Pill>{realtimeVoice ? 'Realtime' : 'Classic'}</Pill>
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            {(
              [
                {
                  id: 'realtime',
                  label: 'Realtime',
                  description: 'Low-latency speech-to-speech. Needs an OpenAI key; falls back to Classic automatically.'
                },
                {
                  id: 'classic',
                  label: 'Classic',
                  description: 'Local transcribe → think → speak loop. Works without a realtime voice key.'
                }
              ] as const
            ).map(option => {
              const active = (realtimeVoice ? 'realtime' : 'classic') === option.id
              ...
              return (
                <button
                  ...
                  onClick={() => {
                    triggerHaptic('selection')
                    setRealtimeVoiceEnabled(option.id === 'realtime')
                  }}
                  type="button"
                >
                  <div className="flex items-start justify-between gap-3">
