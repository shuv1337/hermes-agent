    ...
    func getData(call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        let query = HKSampleQuery(
            ...
            if let quantitySamples = samples as? [HKQuantitySample] {
                let outputUnit = unit ?? HKUnit.internationalUnit()
                let dictionaries = quantitySamples.compactMap { sample -> NSDictionary? in
                    guard sample.quantity.is(compatibleWith: outputUnit) else {
                        NSLog(
                            "HealthDataReader: skipping %@ sample incompatible with %@",
                            dataTypeKey,
                            outputUnit.unitString
                        )
                        return nil
                    }
                    return [
                        "uuid": "\(sample.uuid)",
                        "value": sample.quantity.doubleValue(for: outputUnit),
